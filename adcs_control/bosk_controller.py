...
class AttitudeControlNode(Node):
    ...
    def imu_callback(self, msg):
        ...
        #Control BOSKOVIC
        k_dot = gain_estimator_bosk(self.w, wd, q_err, self.delta, self.gamma, self.k, self.Umax)                # Run adaptation law
        self.k = self.k_ant + self.dt / 6 * (self.k_dot_ant + 2 * (self.k_dot_ant + k_dot) + k_dot)              # Second order Simpson integration
        u = boskovic_control(self.w, wd, q_err, self.delta, self.k, self.Umax)                                   # Calcular ley de control
    
        # Actualizar variables anteriores
        self.k_ant = self.k
        self.k_dot_ant = k_dot
